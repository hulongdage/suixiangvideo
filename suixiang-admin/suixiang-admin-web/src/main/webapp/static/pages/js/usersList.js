...
var List = function () {
	...
	var handleList = function() {
        ...
        jqGrid.jqGrid({  
            caption: "短视频用户列表",  
            url: hdnContextPath + "/user/list.action",  
            mtype: "post",  
            ...
            colModel: [  
                ...
                { name: 'headimage', index: 'headimage', width: 50, sortable: false,
                	formatter:function(cellvalue, options, rowObject) {
                		var src = apiServer + cellvalue;
                		var img = "<img src='" + src + "' height='60' width='120'></img>"
			    		return img;
			    	}  
                },
                { name: 'username', index: 'username', width: 30, sortable: false },
                { name: 'nickname', index: 'nickname', width: 30, sortable: false },
                { name: 'countfan', index: 'countfan', width: 20, sortable: false },
                { name: 'countcon', index: 'countcon', width: 20, sortable: false },
                { name: 'countlove', index: 'countlove', width: 20, sortable: false, hidden: false }
            ],  
        ...
        $("#searchUserListButton").click(function(){
        	...
        	jqGrid.jqGrid().setGridParam({ 
        		page: 1,
                url: hdnContextPath + "/user/list.action?" + searchUsersListForm.serialize(),
            }).trigger("reloadGrid");
        });
    }
